        ...
        <div className="flex flex-wrap gap-3">
          <label className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <select
              ...
              className="bg-white border border-gray-200 rounded-lg px-3 py-2.5 text-sm min-w-[120px] focus:outline-none focus:ring-2 focus:ring-dobby-500/30"
            >
            ...
            {pageItems.map((d) => (
              ...
              >
                <div className="relative h-44 bg-gray-100">
                  ...
                  <span
                    className={`absolute top-3 left-3 px-2.5 py-1 rounded-md text-xs font-semibold backdrop-blur-sm ${
                      STATUS_BADGE[d.status] ?? STATUS_BADGE.OFFLINE
                    }`}
                  >
                    {STATUS_LABELS[d.status] ?? d.status}
                  </span>
                  <div className="absolute top-3 right-3" ref={menuOpenId === d.id ? menuRef : undefined}>
                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="w-8 h-8 rounded-full bg-white/90 shadow border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-white"
                      aria-label="Opciones"
                    >
                ...
                <div className="px-4 pt-4 pb-3">
                  <h2 className="font-semibold text-gray-900 truncate" title={d.name}>
                    {d.name || "—"}
                  </h2>
                  <p className="text-sm text-gray-500 truncate mt-0.5" title={d.user?.email}>
                    {d.user?.email ?? "—"}
                  </p>
                </div>

                <div className="grid grid-cols-3 border-t border-gray-100 mx-4 py-3 gap-2 text-center">
                  <div>
                    <p className="text-[10px] uppercase tracking-wide text-gray-400 font-medium">Pedidos</p>
                    <p className="text-lg font-bold text-gray-900 mt-0.5">{d.totalDeliveries}</p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wide text-gray-400 font-medium">Calificación</p>
                    <p className="text-lg font-bold text-gray-900 mt-0.5 flex items-center justify-center gap-0.5">
                      {d.rating != null ? (
                        <>
                  ...
                  <div>
                    <p className="text-[10px] uppercase tracking-wide text-gray-400 font-medium">Entregas</p>
